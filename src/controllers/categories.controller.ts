  ...
  static createCategory = async (req: Request, res: Response) => {
    try {
      ...
      [data] = await pool.query(
        `SELECT 1 FROM categories WHERE name = ? AND status = 1`,
        [category.name]
      );
      ...
      let checkCategory = data as CategoryModel[];
      let categoryId: any;

      if (checkCategory?.length > 0) { // avoid duplicate insertion
        await transaction(pool, async (connection) => {
          [data] = await connection.query(
            `UPDATE categories SET ? WHERE id = ?`,
            [category, checkCategory[0]?.id]
          );
      ...
      if (categoryId) {
        res
          .status(201)
          .send({ message: `Category with Id: ${categoryId} is created` });
      } else {
  ...
  static updateCategory = async (req: Request, res: Response) => {
    try {
      ...
      if (isUpdated) {
        res
          .status(200)
          .send({ message: `Category with Id: ${categoryId} is updated` });
      } else {
  ...
  static deleteCategory = async (req: Request, res: Response) => {
    try {
      ...
      if (subCategoryExists.length) {
        res
          .status(209)
          .send(`Category with Id: ${categoryId} has sub categories`);
      }
      ...
      if (isDeleted) {
        res
          .status(200)
          .send({ message: `Category with Id: ${categoryId} is deleted` });
      } else {
